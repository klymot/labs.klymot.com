...
class TempChart {
  ...
  // ── AdjChart (vendored from www.klymot.com) ──────────────────────────────────
...
class AdjChart {
  ...
  // ── DualAxisChart ─────────────────────────────────────────────────────────────
  // Dual-Y-axis line chart for labs: left axis (temperature), right axis (solar).
  // API: new DualAxisChart(containerEl) → .load({labels, leftData, rightData,
  //      leftLabel, rightLabel, leftColor, rightColor, labelFilter}) → .destroy()
  ...
  class DualAxisChart {
    constructor(container) {
      ...
      this._leftLabel   = '';
      this._rightLabel  = '';
      this._leftColor   = '#30c880';
      this._rightColor  = '#d4a855';
      this._labelFilter = null;

      this._raf      = null;
      this._geom     = null;
      this._hoverIdx = null;

      this._M = { l: 56, r: 56, t: 14, b: 36 };
    ...
    load({ labels, leftData, rightData, leftLabel, rightLabel, leftColor, rightColor, labelFilter }) {
      this._labels      = labels      || [];
      this._leftData    = leftData    || [];
      this._rightData   = rightData   || [];
      this._leftLabel   = leftLabel   || '';
      this._rightLabel  = rightLabel  || '';
      if (leftColor)   this._leftColor  = leftColor;
      if (rightColor)  this._rightColor = rightColor;
      this._labelFilter = labelFilter || null;
      this._hoverIdx    = null;
      this._tooltip.hidden = true;
      this.resize();
    }
    ...
    _render() {
      ...
      this._geom = { ml: rawM.l, mr: rawM.r, mt: rawM.t, mb: rawM.b,
                     cw: cw / dpr, ch: ch / dpr, n: this._labels.length };
      ...
      const colText = _cssVar('--text-muted')   || '#5a6880';
      const colBg   = _cssVar('--bg-elevated')  || '#152c4a';
      const colInsp = 'rgba(80,144,224,0.5)';
      ...
      const toXi = i => ml + (n > 1 ? i / (n - 1) : 0.5) * cw;
      ...
      const lR = padRange(this._leftData);
      const rR = padRange(this._rightData);

      const toYL = v => mt + (lR.max - v) / (lR.max - lR.min) * ch;
      ...
      ctx.font = `${10 * dpr}px 'JetBrains Mono', monospace`;
      ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
      for (let y = yStartL; y <= lR.max + yStepL * 0.01; y += yStepL) {
        const py = toYL(y);
        if (py < mt - 0.5 || py > mt + ch + 0.5) continue;
        ctx.strokeStyle = colGrid; ctx.lineWidth = 1; ctx.setLineDash([]);
        ctx.beginPath(); ctx.moveTo(ml, py); ctx.lineTo(ml + cw, py); ctx.stroke();
        const dec = Math.max(0, -Math.floor(Math.log10(yStepL)));
        ctx.fillStyle = this._leftColor;
        ctx.fillText(y.toFixed(dec) + '°', ml - 5 * dpr, py);
      }
      ...
      for (let y = yStartR; y <= rR.max + yStepR * 0.01; y += yStepR) {
        const py = toYR(y);
        if (py < mt - 0.5 || py > mt + ch + 0.5) continue;
        const dec = Math.max(0, -Math.floor(Math.log10(yStepR)));
        ctx.fillStyle = this._rightColor;
        ctx.fillText(y.toFixed(dec), ml + cw + 5 * dpr, py);
      }

      // X tick labels
      ctx.textAlign = 'center'; ctx.textBaseline = 'top';
      for (let i = 0; i < n; i++) {
        const label = this._labelFilter ? this._labelFilter(this._labels[i], i) : this._labels[i];
        if (!label) continue;
        ctx.fillStyle = colText;
        ctx.fillText(label, toXi(i), mt + ch + 5 * dpr);
      }

      // Rotated axis title – left
      ctx.save();
      ctx.translate(10 * dpr, mt + ch / 2);
      ctx.rotate(-Math.PI / 2);
      ctx.fillStyle = this._leftColor;
      ctx.font = `${9 * dpr}px 'Source Sans 3', sans-serif`;
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(this._leftLabel, 0, 0);
      ctx.restore();

      // Rotated axis title – right
      ctx.save();
      ctx.translate(W - 10 * dpr, mt + ch / 2);
      ctx.rotate(Math.PI / 2);
      ctx.fillStyle = this._rightColor;
      ctx.font = `${9 * dpr}px 'Source Sans 3', sans-serif`;
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(this._rightLabel, 0, 0);
      ctx.restore();
      ...
      if (this._hoverIdx !== null) {
        const px = toXi(this._hoverIdx);
        ctx.strokeStyle = colInsp; ctx.lineWidth = 1 * dpr; ctx.setLineDash([]);
        ctx.beginPath(); ctx.moveTo(px, mt); ctx.lineTo(px, mt + ch); ctx.stroke();
      }

      // Draw a line series
      const drawLine = (data, toY, color, lw) => {
        ...
        for (let i = 0; i < n; i++) {
          ...
          if (v == null || !Number.isFinite(v)) {
            if (open) { ctx.stroke(); ctx.beginPath(); open = false; } continue;
          }
          const px = toXi(i), py = toY(v);
          if (!open) { ctx.moveTo(px, py); open = true; } else { ctx.lineTo(px, py); }
        }
        if (open) ctx.stroke();
      };

      drawLine(this._leftData,  toYL, this._leftColor,  2);
      drawLine(this._rightData, toYR, this._rightColor, 2);

      // Hover dots
      if (this._hoverIdx !== null) {
        const dot = (data, toY, color) => {
          const v = data[this._hoverIdx];
          if (v == null || !Number.isFinite(v)) return;
          ctx.beginPath(); ctx.arc(toXi(this._hoverIdx), toY(v), 3.5 * dpr, 0, Math.PI * 2);
          ctx.fillStyle = color; ctx.fill();
          ctx.strokeStyle = colBg; ctx.lineWidth = 1.5 * dpr; ctx.stroke();
        };
        dot(this._leftData,  toYL, this._leftColor);
        dot(this._rightData, toYR, this._rightColor);
      }
    ...
    _onMouseMove(e) {
      if (!this._geom || !this._labels.length) return;
      const rect   = this._canvas.getBoundingClientRect();
      const mouseX = e.clientX - rect.left;
      const mouseY = e.clientY - rect.top;
      const { ml, mt, cw, ch, n } = this._geom;

      if (mouseX < ml || mouseX > ml + cw || mouseY < mt || mouseY > mt + ch) {
        if (this._hoverIdx !== null) { this._hoverIdx = null; this._tooltip.hidden = true; this._scheduleRender(); }
        return;
      }

      const idx = Math.max(0, Math.min(n - 1, Math.round((mouseX - ml) / cw * (n - 1))));
      if (this._hoverIdx === idx) return;
      this._hoverIdx = idx;

      const lv = this._leftData[idx];
      const rv = this._rightData[idx];
      const lines = [this._labels[idx]];
      if (lv != null && Number.isFinite(lv)) lines.push(`${this._leftLabel || 'Left'}: ${lv.toFixed(2)}°C`);
      if (rv != null && Number.isFinite(rv)) lines.push(`${this._rightLabel || 'Right'}: ${rv.toFixed(1)}`);
      this._tooltip.textContent = lines.join('\n');
      this._tooltip.hidden = false;

      const px = ml + idx / (n - 1) * cw;
      const ttW = this._tooltip.offsetWidth + 4;
      const left = px + 14 + ttW > rect.width ? px - ttW - 8 : px + 14;
      const top  = Math.max(4, Math.min(mouseY - 18, rect.height - (this._tooltip.offsetHeight || 28) - 4));
      this._tooltip.style.left = left + 'px';
  ...
  // ── ScatterChart ──────────────────────────────────────────────────────────────
  // Scatter plot with optional regression fit line for labs pages.
  // API: new ScatterChart(containerEl) → .load({points, fitLine, xLabel, yLabel,
  //      pointColor, fitColor}) → .destroy()
  // points: [{x, y, label?}], fitLine: {points:[{x,y}]} | null
  ...
  class ScatterChart {
    constructor(container) {
      ...
      this._geom    = null;
      this._hoverPt = null;
      this._xMin2 = 0; this._xMax2 = 1;
      this._yMin2 = 0; this._yMax2 = 1;

      this._M = { l: 52, r: 16, t: 12, b: 40 };
    ...
    _render() {
      ...
      this._geom = { ml: rawM.l, mr: rawM.r, mt: rawM.t, mb: rawM.b, cw: cw / dpr, ch: ch / dpr };
      ...
      const pts = this._points.filter(p => Number.isFinite(p.x) && Number.isFinite(p.y));
      ...
      this._xMin2 = xMin; this._xMax2 = xMax;
      this._yMin2 = yMin; this._yMax2 = yMax;
      ...
      for (let y = yStart; y <= yMax + yStep * 0.01; y += yStep) {
        ...
        ctx.beginPath(); ctx.moveTo(ml, py); ctx.lineTo(ml + cw, py); ctx.stroke();
        const dec = Math.max(0, -Math.floor(Math.log10(yStep)));
        ctx.fillStyle = colText; ctx.fillText(y.toFixed(dec), ml - 5 * dpr, py);
      }
      ...
      for (let x = xStart; x <= xMax + xStep * 0.01; x += xStep) {
        ...
        ctx.strokeStyle = colGrid; ctx.lineWidth = 1; ctx.setLineDash([]);
        ctx.beginPath(); ctx.moveTo(px, mt); ctx.lineTo(px, mt + ch); ctx.stroke();
        const dec = Math.max(0, -Math.floor(Math.log10(xStep)));
        ctx.fillStyle = colText; ctx.fillText(x.toFixed(dec), px, mt + ch + 5 * dpr);
      }

      // Axis labels
      if (this._xLabel) {
        ctx.fillStyle = colText; ctx.font = `${9 * dpr}px 'Source Sans 3', sans-serif`;
        ctx.textAlign = 'center'; ctx.textBaseline = 'bottom';
        ctx.fillText(this._xLabel, ml + cw / 2, H - 2 * dpr);
      }
      if (this._yLabel) {
        ctx.save();
        ctx.translate(10 * dpr, mt + ch / 2); ctx.rotate(-Math.PI / 2);
        ctx.fillStyle = colText; ctx.font = `${9 * dpr}px 'Source Sans 3', sans-serif`;
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ...
      // Fit line
      if (this._fitLine && this._fitLine.points && this._fitLine.points.length >= 2) {
        const fp = this._fitLine.points;
        ctx.strokeStyle = this._fitColor; ctx.lineWidth = 2 * dpr; ctx.setLineDash([]);
        ctx.lineJoin = 'round'; ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(toX(fp[0].x), toY(fp[0].y));
        for (let i = 1; i < fp.length; i++) ctx.lineTo(toX(fp[i].x), toY(fp[i].y));
        ctx.stroke();
      }
      ...
      if (this._hoverPt) {
        ...
        ctx.beginPath(); ctx.arc(toX(p.x), toY(p.y), 5 * dpr, 0, Math.PI * 2);
    ...
    _onMouseMove(e) {
      if (!this._geom || !this._points.length) return;
      const rect   = this._canvas.getBoundingClientRect();
      const mouseX = e.clientX - rect.left;
      const mouseY = e.clientY - rect.top;
      const { ml, mt, cw, ch } = this._geom;
      ...
      if (mouseX < ml || mouseX > ml + cw || mouseY < mt || mouseY > mt + ch) {
        if (this._hoverPt) { this._hoverPt = null; this._tooltip.hidden = true; this._scheduleRender(); }
        return;
      }

      const xD = this._xMin2 + (mouseX - ml) / cw * (this._xMax2 - this._xMin2);
      const yD = this._yMax2 - (mouseY - mt) / ch * (this._yMax2 - this._yMin2);

      const toPx = (x) => (x - this._xMin2) / (this._xMax2 - this._xMin2) * cw;
      const toPy = (y) => (this._yMax2 - y) / (this._yMax2 - this._yMin2) * ch;

      let best = null, bestD = Infinity;
      for (const p of this._points) {
        if (!Number.isFinite(p.x) || !Number.isFinite(p.y)) continue;
        const dx = toPx(p.x) - toPx(xD), dy = toPy(p.y) - toPy(yD);
        const d = dx * dx + dy * dy;
        if (d < bestD) { bestD = d; best = p; }
      }

      if (!best || Math.sqrt(bestD) > 20) {
        if (this._hoverPt) { this._hoverPt = null; this._tooltip.hidden = true; this._scheduleRender(); }
        return;
      }
      ...
      const px  = ml + (best.x - this._xMin2) / (this._xMax2 - this._xMin2) * cw;
      const ttW = this._tooltip.offsetWidth + 4;
      const left = px + 14 + ttW > rect.width ? px - ttW - 8 : px + 14;
      const top  = Math.max(4, Math.min(mouseY - 18, rect.height - (this._tooltip.offsetHeight || 28) - 4));
      this._tooltip.style.left = left + 'px';
      ...
      if (this._hoverPt !== prev) this._scheduleRender();
    }
  ...
  // ── Export ────────────────────────────────────────────────────────────────────
  root.KlymotChart = {
    TempChart, AdjChart, DualAxisChart, ScatterChart,
    MONTHS, MONTH_COLORS_DARK, MONTH_COLORS_LIGHT, MONTH_DASH, BYMONTH_DEFAULT_MASK,
  };
